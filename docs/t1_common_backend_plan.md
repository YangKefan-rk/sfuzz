...
| --- | --- |
| `testcase_id` | 稳定 ID |
| `source_kind` | `sfuz`、`litmus`、`raw-pin-stream`、`direct-target`、`surge-annotation` 等 |
| `source_path` | 原始输入路径 |
...
| `RFuzz` | RFuzz 生成的输入必须保存为可审计 testcase，并由 sub5 映射到同一 LinkNan DUT 运行 | 当前脚本可用 SFUZ core0 payload 做 smoke，但不是 RFuzz paper-faithful raw pin-stream |
