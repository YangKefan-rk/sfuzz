...
本节区分“已经能真实运行的工程路径”和“论文主工具还必须补齐的 native feedback
闭环”。当前 SFuzz 主仓已有这些能力：

- Rust 主入口支持 `--coverage <name>`、`--fuzzing`、`--max-iters`、`--corpus-input`、
  `--corpus-output`、`--continue-on-errors`、`--save-errors`，并通过 LibAFL
  `StdMapObserver + MaxMapFeedback + HavocScheduledMutator + QueueScheduler` 做基础覆盖导向 fuzz。
- Verilator/FUZZER_LIB 路径已经接到 `sim_main_with_input`，可以把 LibAFL `BytesInput`
  直接送入 LinkNan 的 `sfuzz-abi-buffer`，由 `ram.cpp` 识别 `SFUZ` 并展开到 RAM。
- VCS 路径是 file-based：`scripts/linknan/run.py sfuzz` 调用真实
  `xmake simv-run --workload=<seed.sfuz>`，可以批量跑真实 `.sfuz` corpus 并记录
  `SFuzz structured seed detected`、VCS report、周期上限、assert log 和可选 VCS
  built-in coverage。
- `SFUZ` v1 codec 已覆盖 `core0_prog`、`core1_prog`、shared memory segment、
  interrupt plan 和 metadata；LinkNan `ram.cpp` 当前会装载 core0、暂存 core1、
  装载 shared segment，并跳过 interrupt metadata。
- `coverage_strategy` 目前只是把命名覆盖后端传给 simulator：`llvm.branch`、
  `Instruction`、`Instr-Imm`、`FIRRTL.<group>` 等依赖 LinkNan/DiffTest 编译时暴露。
  `union:<a>+<b>` 只有在 simulator 真的把 `UnionCoverage` 注册到 `stats.cover` 时才可用；
  当前可见 LinkNan `dut.h` 中 union coverage 实例仍是注释状态，因此 union 不能算已完成闭环。
- 当前可见的 FIRRTL 覆盖库存来自
  [firrtl_litmus_coverage_report.md](firrtl_litmus_coverage_report.md)：`Directory_1`、
  `MSHR`、`RXSNP`、`SinkA`、`SinkC`、`SourceB` 共 23 点。这些点足以作为
  `SFUZZ.coherence.P0.initial` 的雏形，但只覆盖 L2/CHI/coherence 子集。

尚未完成、不能写成论文主工具完备结果的部分：

- VCS 尚未把本文定义的 `sfuzz_linknan_native` per-seed local bitmap 通过 DPI/PLI
  导出给 SFuzz runner；当前 VCS 批处理只能证明真实 seed 被 LinkNan 接收并运行，或读取
  VCS built-in `.vdb`，不能替代 SFuzz native microarchitectural feedback。
- 当前 23 个 FIRRTL 点不足以覆盖本文 P0 的前端、分支重定向、MMU/PTW、ROB/提交、
  CSR/中断、AMO/LRSC/Fence 等主工具反馈面；它们更适合作为 coherence 初版或
  `T1_common_backend`。
- Rust `run_fuzzer` 仍使用 LibAFL 默认 havoc byte 变异。它能变异 `.sfuz` 字节流，
  但还没有结构感知的 SFUZ 变异算子，例如保留 header/length、按 RISC-V 指令替换、
  shared segment 地址桶化、interrupt plan 插入/删除、core0/core1 协同变异。
- `QueueScheduler` 是基础 FIFO 调度；还没有面向 SFuzz 主线的结构化 seed 调度，
  例如按新 microarchitectural group、valid-seed 深度、trap/commit/coherence 目标或
  seed section 类型做能量分配。
- LinkNan `ram.cpp` 目前会暂存 core1 payload，但日志已说明 execution handoff
  尚未启用；因此多核 `.sfuz` seed 的双核程序语义还不能直接视为完整执行。
...
当前已证明可落地的覆盖点只有 coherence 初版 23 点，因此“首批覆盖集”的执行顺序应按
下面两阶段推进：

1. `SFUZZ.coherence.P0.initial`：复用已生成过的 `Directory_1/MSHR/RXSNP/SinkA/SinkC/SourceB`
   23 点，先完成 metadata、local bitmap reset、VCS/Verilator 导出和 runner 统计。
2. `SFUZZ.arch.P0`：再补前端、分支、MMU/PTW、ROB/CSR/中断等 40-80 个点。只有这一步完成后，
   SFuzz native feedback 才接近论文主工具所需的系统级完整面。
