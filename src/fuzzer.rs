...
fn populate_initial_corpus<E, EM, Z>(
    ...
    EM: EventFirer<BytesInput, FuzzState>,
    Z: Evaluator<E, EM, BytesInput, FuzzState>,
{
    ...
    match corpus_input {
        Some(corpus_path) => {
            let corpus_dirs = vec![PathBuf::from(corpus_path)];
            state
...
pub(crate) fn run_directed_fuzzer(
    ...
    use crate::directed::DirectedScheduler;

    // DirectFuzz Scheduler (weighted by module hierarchy distance)
    let scheduler = DirectedScheduler::new(&target_module);
    let observer =
        unsafe { StdMapObserver::from_mut_ptr("signals", cover_as_mut_ptr(), cover_len()) };
    let mut feedback = MaxMapFeedback::new(&observer);
    let mut objective = CrashFeedback::new();
    ...
    // Fuzzer, Executor (same as RFuzz baseline)
    let mut fuzzer = StdFuzzer::new(scheduler, feedback, objective);
    let mut binding = harness::fuzz_harness;
    let mut executor = InProcessExecutor::with_timeout(
        &mut binding,
